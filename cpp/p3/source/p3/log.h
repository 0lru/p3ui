...
#define log_fatal(text, ...)                                        \
    if constexpr (::logger::level::current >= logger::level::fatal) \
    ::logger::fatal(__FILE__, __LINE__, text, __VA_ARGS__)
#define log_error(text, ...)                                        \
    if constexpr (::logger::level::current >= logger::level::error) \
    ::logger::error(__FILE__, __LINE__, text, __VA_ARGS__)
#define log_warn(text, ...)                                        \
    if constexpr (::logger::level::current >= logger::level::warn) \
    ::logger::warn(__FILE__, __LINE__, text, __VA_ARGS__)
#define log_info(text, ...)                                        \
    if constexpr (::logger::level::current >= logger::level::info) \
    ::logger::info(__FILE__, __LINE__, text, __VA_ARGS__)
#define log_debug(text, ...)                                        \
    if constexpr (::logger::level::current >= logger::level::debug) \
    ::logger::debug(__FILE__, __LINE__, text, __VA_ARGS__)
#define log_verbose(text, ...)                                        \
    if constexpr (::logger::level::current >= logger::level::verbose) \
    ::logger::verbose(__FILE__, __LINE__, text, __VA_ARGS__)
#define log_with_level(custom_level, text, ...)             \
    if constexpr (::logger::level::current >= custom_level) \
    ::logger::info(__FILE__, __LINE__, text, __VA_ARGS__)
